...
from financial_forecasting_platform.features.engineering import (
    create_garman_klass_variance,
    create_gk_variance_rolling_mean,
    create_parkinson_variance,
    create_parkinson_variance_rolling_mean,
    create_parkinson_volatility,
    create_rogers_satchell_variance,
    create_rogers_satchell_variance_rolling_mean,
    create_rogers_satchell_volatility,
    create_yang_zhang_variance,
    create_yang_zhang_variance_rolling_mean,
    create_yang_zhang_volatility,
...
class TestCreateGarmanKlassVariance:
    def test_adds_column(self, sample_ohlcv_df):
        result = create_garman_klass_variance(sample_ohlcv_df, lag=1)
        assert "gk_variance_lag_1" in result.columns

    def test_column_name_with_lag(self, sample_ohlcv_df):
        result = create_garman_klass_variance(sample_ohlcv_df, lag=3)
    ...
    def test_invalid_lag_raises(self, sample_ohlcv_df):
        with pytest.raises(ValueError):
            create_garman_klass_variance(sample_ohlcv_df, lag=0)

    def test_does_not_modify_original(self, sample_ohlcv_df):
        original = sample_ohlcv_df.copy()
        create_garman_klass_variance(sample_ohlcv_df)
        pd.testing.assert_frame_equal(sample_ohlcv_df, original)


class TestCreateGkVarianceRollingMean:
    def test_adds_column(self, sample_ohlcv_df):
        result = create_gk_variance_rolling_mean(sample_ohlcv_df, window_size=5, lag=1)
        assert "gk_variance_mean_5_lag_1" in result.columns

    def test_column_name_with_params(self, sample_ohlcv_df):
        result = create_gk_variance_rolling_mean(sample_ohlcv_df, window_size=10, lag=3)
        assert "gk_variance_mean_10_lag_3" in result.columns

    def test_values_are_rolling_mean_of_gk_variance(self, sample_ohlcv_df):
        window, lag = 5, 1
        result = create_gk_variance_rolling_mean(sample_ohlcv_df, window_size=window, lag=lag)
        gk = create_garman_klass_variance(sample_ohlcv_df, lag=lag)
        for ticker in sample_ohlcv_df["Ticker"].unique():
    ...
    def test_invalid_window_raises(self, sample_ohlcv_df):
        with pytest.raises(ValueError):
            create_gk_variance_rolling_mean(sample_ohlcv_df, window_size=0, lag=1)

    def test_invalid_lag_raises(self, sample_ohlcv_df):
        with pytest.raises(ValueError):
            create_gk_variance_rolling_mean(sample_ohlcv_df, window_size=2, lag=0)

    def test_does_not_modify_original(self, sample_ohlcv_df):
        original = sample_ohlcv_df.copy()
        create_gk_variance_rolling_mean(sample_ohlcv_df)
        pd.testing.assert_frame_equal(sample_ohlcv_df, original)


class TestCreateParkinsonVariance:
    def test_adds_column(self, sample_ohlcv_df):
        result = create_parkinson_variance(sample_ohlcv_df, lag=1)
    ...
    def test_invalid_lag_raises(self, sample_ohlcv_df):
        with pytest.raises(ValueError):
            create_parkinson_variance(sample_ohlcv_df, lag=0)

    def test_does_not_modify_original(self, sample_ohlcv_df):
        original = sample_ohlcv_df.copy()
        create_parkinson_variance(sample_ohlcv_df)
        pd.testing.assert_frame_equal(sample_ohlcv_df, original)


class TestCreateParkinsonVarianceRollingMean:
    def test_adds_column(self, sample_ohlcv_df):
        result = create_parkinson_variance_rolling_mean(sample_ohlcv_df, window_size=5, lag=1)
        assert "parkinson_variance_mean_5_lag_1" in result.columns

    def test_values_are_rolling_mean(self, sample_ohlcv_df):
        window, lag = 5, 1
        result = create_parkinson_variance_rolling_mean(sample_ohlcv_df, window_size=window, lag=lag)
        pv = create_parkinson_variance(sample_ohlcv_df, lag=lag)
        for ticker in sample_ohlcv_df["Ticker"].unique():
    ...
    def test_invalid_window_raises(self, sample_ohlcv_df):
        with pytest.raises(ValueError):
            create_parkinson_variance_rolling_mean(sample_ohlcv_df, window_size=0, lag=1)

    def test_invalid_lag_raises(self, sample_ohlcv_df):
        with pytest.raises(ValueError):
            create_parkinson_variance_rolling_mean(sample_ohlcv_df, window_size=2, lag=0)

    def test_does_not_modify_original(self, sample_ohlcv_df):
        original = sample_ohlcv_df.copy()
        create_parkinson_variance_rolling_mean(sample_ohlcv_df)
        pd.testing.assert_frame_equal(sample_ohlcv_df, original)


class TestCreateParkinsonVolatility:
    def test_adds_column(self, sample_ohlcv_df):
        result = create_parkinson_volatility(sample_ohlcv_df, window_size=5, lag=1)
    ...
    def test_values_are_sqrt_of_rolling_mean_variance(self, sample_ohlcv_df):
        window, lag = 5, 1
        result = create_parkinson_volatility(sample_ohlcv_df, window_size=window, lag=lag)
        rm = create_parkinson_variance_rolling_mean(sample_ohlcv_df, window_size=window, lag=lag)
        for ticker in sample_ohlcv_df["Ticker"].unique():
    ...
    def test_does_not_modify_original(self, sample_ohlcv_df):
        original = sample_ohlcv_df.copy()
        create_parkinson_volatility(sample_ohlcv_df)
        pd.testing.assert_frame_equal(sample_ohlcv_df, original)


class TestCreateRogersSatchellVariance:
    def test_adds_column(self, sample_ohlcv_df):
        result = create_rogers_satchell_variance(sample_ohlcv_df, lag=1)
    ...
    def test_invalid_lag_raises(self, sample_ohlcv_df):
        with pytest.raises(ValueError):
            create_rogers_satchell_variance(sample_ohlcv_df, lag=0)

    def test_does_not_modify_original(self, sample_ohlcv_df):
        original = sample_ohlcv_df.copy()
        create_rogers_satchell_variance(sample_ohlcv_df)
        pd.testing.assert_frame_equal(sample_ohlcv_df, original)


class TestCreateRogersSatchellVarianceRollingMean:
    def test_adds_column(self, sample_ohlcv_df):
        result = create_rogers_satchell_variance_rolling_mean(sample_ohlcv_df, window_size=5, lag=1)
        assert "rs_variance_mean_5_lag_1" in result.columns

    def test_values_are_rolling_mean(self, sample_ohlcv_df):
        window, lag = 5, 1
        result = create_rogers_satchell_variance_rolling_mean(sample_ohlcv_df, window_size=window, lag=lag)
        rs = create_rogers_satchell_variance(sample_ohlcv_df, lag=lag)
        for ticker in sample_ohlcv_df["Ticker"].unique():
    ...
    def test_does_not_modify_original(self, sample_ohlcv_df):
        original = sample_ohlcv_df.copy()
        create_rogers_satchell_variance_rolling_mean(sample_ohlcv_df)
        pd.testing.assert_frame_equal(sample_ohlcv_df, original)


class TestCreateRogersSatchellVolatility:
    def test_adds_column(self, sample_ohlcv_df):
        result = create_rogers_satchell_volatility(sample_ohlcv_df, window_size=5, lag=1)
        assert "rs_volatility_5_lag_1" in result.columns

    def test_values_are_sqrt_of_rolling_mean_variance(self, sample_ohlcv_df):
        window, lag = 5, 1
        result = create_rogers_satchell_volatility(sample_ohlcv_df, window_size=window, lag=lag)
        rm = create_rogers_satchell_variance_rolling_mean(sample_ohlcv_df, window_size=window, lag=lag)
        for ticker in sample_ohlcv_df["Ticker"].unique():
...
class TestCreateYangZhangVariance:
    ...
    def test_invalid_lag_raises(self, single_ticker_df):
        with pytest.raises(ValueError):
            create_yang_zhang_variance(single_ticker_df, lag=0)

    def test_does_not_modify_original(self, single_ticker_df):
        original = single_ticker_df.copy()
        create_yang_zhang_variance(single_ticker_df)
        pd.testing.assert_frame_equal(single_ticker_df, original)


class TestCreateYangZhangVarianceRollingMean:
    def test_adds_column(self, single_ticker_df):
        result = create_yang_zhang_variance_rolling_mean(single_ticker_df, lag=1, window_size=5)
        assert "yz_variance_mean_5_lag_1" in result.columns

    def test_values_are_rolling_mean(self, single_ticker_df):
        window, lag = 5, 1
        result = create_yang_zhang_variance_rolling_mean(single_ticker_df, lag=lag, window_size=window)
        yz = create_yang_zhang_variance(single_ticker_df, lag=lag)
    ...
    def test_invalid_window_raises(self, single_ticker_df):
        with pytest.raises(ValueError):
            create_yang_zhang_variance_rolling_mean(single_ticker_df, lag=1, window_size=0)

    def test_does_not_modify_original(self, single_ticker_df):
        original = single_ticker_df.copy()
        create_yang_zhang_variance_rolling_mean(single_ticker_df)
        pd.testing.assert_frame_equal(single_ticker_df, original)


class TestCreateYangZhangVolatility:
    def test_adds_column(self, single_ticker_df):
        result = create_yang_zhang_volatility(single_ticker_df, lag=1, window_size=5)
        assert "yz_volatility_5_lag_1" in result.columns

    def test_values_are_sqrt_of_variance(self, single_ticker_df):
        window, lag = 5, 1
        result = create_yang_zhang_volatility(single_ticker_df, lag=lag, window_size=window)
        rm = create_yang_zhang_variance_rolling_mean(single_ticker_df, lag=lag, window_size=window)
    ...
    def test_does_not_modify_original(self, single_ticker_df):
        original = single_ticker_df.copy()
        create_yang_zhang_volatility(single_ticker_df)
        pd.testing.assert_frame_equal(single_ticker_df, original)
...
class TestCreateYangZhangVolatilityRatioFeature:
    ...
    def test_ratio_values(self, single_ticker_df):
        ...
        vol_short = create_yang_zhang_volatility(single_ticker_df, lag=1, window_size=short)
        vol_long = create_yang_zhang_volatility(single_ticker_df, lag=1, window_size=long)
