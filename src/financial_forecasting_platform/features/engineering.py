...
def create_market_movement_target(df: pd.DataFrame) -> pd.Series:
    ...
    return pd.Series(market_movement, name="market_movement")
...
def create_close_difference_feature(df: pd.DataFrame, lag: int = 1) -> pd.DataFrame:
    if lag < 1:
    ...
    column_name = f"close_difference_{lag}"
